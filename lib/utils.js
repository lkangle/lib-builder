...
const BABEL_CONFIG_FILENAMES = [".babelrc", ".babelrc.js", ".babelrc.cjs", ".babelrc.mjs", ".babelrc.json",
  "babel.config.js", "babel.config.cjs", "babel.config.mjs", "babel.config.json"];
const ESLINT_CONFIG_FILENAMES = [".eslintrc", ".eslintrc.js", ".eslintrc.cjs", ".eslintrc.yaml", ".eslintrc.yml", ".eslintrc.json"]

const TEMP_FOLDER = path.resolve(__dirname, '../templates')
const CWD = process.cwd()

// 命令优先 获取serve服务contentBase地址
const serverContentBase = (output, contentBase) => {
  if (!output) return contentBase
  let base = path.basename(contentBase)
  let folder = path.dirname(output).replace(/^\//, '')
...
const existsAnyFiles = (...files) => {
  return files.some(file => fs.existsSync(path.resolve(CWD, file)))
}
