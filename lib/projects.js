...
async function createProject(name, options) {
  let [projectPath, projectName] = getProjectInfo(name)
...
async function initProject(name) {
  ...
  await unzipFile(projectPath, path.resolve(TEMP_FOLDER, 'base.zip'))
  extendFieldOfPkgJson(projectPath, 'name', projectName)

  console.log(chalk.green(`\n 项目【${projectName}】创建成功！`))
  console.log(chalk.green(`   安装依赖: yarn`))
  console.log(chalk.green(`   运行测试: yarn dev`))
  console.log(chalk.green(`   运行打包: yarn build`))
  console.log(chalk.green(` -> ${projectPath}`))
}
