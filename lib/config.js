...
const {
  CWD, BABEL_CONFIG_FILENAMES,
  ESLINT_CONFIG_FILENAMES,
  serverContentBase, existsAnyFiles,
} = require('./utils')
...
function buildPlugins(config, { watch, output }) {
  ...
  _pushNotExist(userPlugins, json())
  _pushNotExist(userPlugins, alias({
    entries: config.alias
  }))

  if (existsAnyFiles(...ESLINT_CONFIG_FILENAMES)) {
    const eslint = require('@rollup/plugin-eslint')
    _pushNotExist(userPlugins, eslint({
      throwOnError: true
    }))
  }

  if (existsAnyFiles(...BABEL_CONFIG_FILENAMES)) {
  ...
  if (watch) {
    ...
    let dist = serverContentBase(output, config.server.contentBase)
    let serveConfig = { ...config.server, contentBase: [`${CWD}/test`, dist] }
